...
class WPLPAdmin
{
     ...
     *
     * @param array $messages List message display
     *
     * @return mixed
     */
     ...
     *
     * @return integer $post_id (unchanged)
     */
    public function saveCustomPostdata($post_id)
    {
        ...
        // phpcs:ignore WordPress.Security.NonceVerification -- Nonce is verified in next line
            ...
            if (preg_match('/^wplp_/', $field_name)) {
                ...
                if (is_uploaded_file($_FILES[$field_name]['tmp_name'])) {
                    ...
                    if (rename($_FILES[$field_name]['tmp_name'], $upload_dir . $upload_file)
                        ...
                        $this->update_msg .= __('Processing of temporary uploader file has failed please check for file directory ', 'wp-latest-posts') . $upload_dir;
    ...
    public function loadAdminScripts($hook)
    {
        ...
        wp_localize_script('wplp-back', 'wpsolBackJS', array('ajaxnonce' => $ajax_non));
    ...
    public function setupCustomMenu()
    {
        add_submenu_page(
            ...
            'translation-wpfpn',
            array(
                new WPLPViewsConfig($this->version),
                'displayConfigTab'
            )
        );
    }
