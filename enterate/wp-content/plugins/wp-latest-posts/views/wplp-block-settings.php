...
class WPLPBlockSettings
{
    ...
    private function displayImageSourceTab()
    {
        ...
        echo '<li class="field input-field input-select width33"><label for="thumb_width" class="coltab">' .
            esc_html__('Image size', 'wp-latest-posts') . '</label>' .
            '<select id="wplp_imageThumbSize" name="wplp_image_size">
				<option  ' . esc_attr($imageThumbSizeSelected) . ' value="thumbnailSize" >' .
            esc_html__('Thumbnail', 'wp-latest-posts') . '</option>

				<option  ' . esc_attr($imageMediumSizeSelected) . ' value="mediumSize" >' .
            esc_html__('Medium', 'wp-latest-posts') . '</option>

                <option  ' . esc_attr($imageLargeSizeSelected) . ' value="largeSize" >' .
            esc_html__('Large', 'wp-latest-posts') . '</option>

			</select></li>';
        /*
        * display image width for default theme
        */
        echo '<li class="field ' . esc_attr($class_enabled_default) . '">
        <label for="image_position_width" class="coltab wplp_tooltip" alt="' .
            esc_html__('The image width in %, used when the image is loaded on left position', 'wp-latest-posts') . '">' .
            esc_html__('Image position width', 'wp-latest-posts') . '</label>' .
            '<input id="image_position_width" ' . esc_attr($disabled_image_postion) .
            ' size="20" type="text" style="width:50px" name="wplp_image_position_width" value="' .
            esc_html(htmlspecialchars(isset($settings['image_position_width']) ? $settings['image_position_width'] : '')) .
            '" class="short-text ' . esc_attr($class_enabled_default) . '" />';
        ...
        echo '<li class="field"><label class="coltab">' .
            esc_html__('Full height images', 'wp-latest-posts') . '</label>' .
            '<span class="radioset">' .
            '<input id="full_height1" type="radio" name="wplp_full_height" value="0" ' .
            (isset($full_height_checked[0]) ? esc_attr($full_height_checked[0]) : '') . '/>' .
            '<label for="full_height1">' . esc_html__('Off', 'wp-latest-posts') . '</label>' .
            '<input id="full_height2" type="radio" name="wplp_full_height" value="1" ' .
            (isset($full_height_checked[1]) ? esc_attr($full_height_checked[1]) : '') . '/>' .
            '<label for="full_height2">' . esc_html__('On', 'wp-latest-posts') . '</label>' .
            '</span></li>';
        ...
        if (!class_exists('WPLPAddonAdmin')) {
            echo '<div class="card wplp_notice light-blue notice notice-success is-dismissible below-h2">' .
                ...
                esc_html__('Additional advanced customization features are available with the optional ', 'wp-latest-posts') .
                '<a href="http://www.joomunited.com/wordpress-products/wp-latest-posts" target="_blank" >'.
                esc_html__('pro add-on', 'wp-latest-posts').
                '</a></div></div>';
        } else {
    ...
    private function displayAdvancedTab()
    {
        ...
        echo '<li class="field"><label for="date_fmt" class="coltab">' .
            esc_html__('Date format', 'wp-latest-posts') . '</label>' .
            '<input id="date_fmt" type="text" name="wplp_date_fmt" value="' .
            esc_html(htmlspecialchars(isset($settings['date_fmt']) ? $settings['date_fmt'] : '')) .
            '" class="short-text" />
			<a id="wplp_dateFormat" target="_blank" href="http://php.net/manual/en/function.date.php"> ' .
            esc_html__('Date format', 'wp-latest-posts') . ' </a>
			</li>';

        echo '<li class="field"><label for="text_content" class="coltab">' .
            esc_html__('Text Content', 'wp-latest-posts') . '</label>' .
            ...
            ' class="short-text">' . esc_html__('Full content', 'wp-latest-posts') . '</option>' .
            '<option value="1" ' . ((isset($settings['text_content']) &&
                $settings['text_content'] === '1') ? 'selected' : '') .
            ' class="short-text">' . esc_html__('Excerpt content', 'wp-latest-posts') . '</option>' .
            '</select> </li>';

        echo '<li class="field"><label for="no_post_text" class="coltab">' .
            esc_html__('No post text', 'wp-latest-posts') . '</label>' .
            '<input id="no_post_text" type="text" name="wplp_no_post_text" value="' .
            esc_html(htmlspecialchars(isset($settings['no_post_text']) ? $settings['no_post_text'] : '')) .
            '" class="short-text" /></li>';
        ...
        echo '<hr/><div><label for="custom_css" class="coltab" style="vertical-align:top">' .
            esc_html__('Custom CSS', 'wp-latest-posts') . '</label>' .
            '<textarea id="wplp_custom_css" cols="100" rows="5" name="wplp_custom_css">' .
            (isset($settings['custom_css']) ? esc_html($settings['custom_css']) : '') . '</textarea></div>';
        ...
        if (isset($post->ID) && isset($post->post_title) && (!empty($post->post_title))) {
            echo '<hr style="clear:both"/><div><label for="phpCodeInsert" class="coltab" style="margin:10px 0 5px">' .
                esc_html__('Copy & paste this code into a template file to display this WPLP block', 'wp-latest-posts') .
                '</label>' .
                '<br><textarea readonly id="phpCodeInsert" cols="100" rows="2" name="wplp_phpCodeInsert">' .
                'echo do_shortcode(\'[frontpage_news widget="' . esc_html($post->ID) . '" name="' .
        ...
        if (!class_exists('WPLPAddonAdmin')) {
            echo '<div class="card wplp_notice light-blue notice notice-success is-dismissible below-h2">' .
                '<div class="card-content white-text">';
            echo '<p>' . esc_html__('Looking out for more advanced features?', 'wp-latest-posts') . '</p>';
            echo '<p>' . esc_html__('Check out our optional', 'wp-latest-posts') .
            '<a href="http://www.joomunited.com/wordpress-products/wp-latest-posts" target="_blank" style="text-decoration:none;">'.
                esc_html__('Pro add-on', 'wp-latest-posts') .
                '</a></p>';
            echo '</div></div>';
        }
    ...
    private function displayDisplayThemeTab()
    {
         ...
         */
        echo '<li class="field"><label class="coltab">' . esc_html__('Show title', 'wp-latest-posts') . '</label>' .
            '<span class="radioset">' .
            '<input id="show_title1" type="radio" name="wplp_show_title" value="0" ' .
            (isset($show_title_checked[0]) ? esc_html($show_title_checked[0]) : '') . '/>' .
            '<label for="show_title1">' . esc_html__('Off', 'wp-latest-posts') . '</label>' .
            '<input id="show_title2" type="radio" name="wplp_show_title" value="1" ' .
            (isset($show_title_checked[1]) ? esc_html($show_title_checked[1]) : '') . '/>' .
            '<label for="show_title2">' . esc_html__('On', 'wp-latest-posts') . '</label>' .
            '</span>';
        echo '</li>';
         ...
         */
        echo '<li class="field ' . esc_attr($classdisabledsmooth) . '">
<label for="   amount_cols" class="coltab">' .
            esc_html__('Number of columns', 'wp-latest-posts') . '</label>' .
            '<input id="amount_cols" type="text" name="wplp_amount_cols" value="' .
            esc_html(htmlspecialchars(isset($settings['amount_cols']) ? $settings['amount_cols'] : '3')) .
            '" class="short-text" ' . esc_attr($classdisabledsmooth) . '/></li>';
        /*
         * display number of rows
         */
        echo '<li class="field ' . esc_attr($classdisabled) . esc_attr($classdisabledsmooth) . '">
<label for="amount_rows" class="coltab">' .
            esc_html__('Number of rows', 'wp-latest-posts') . '</label>' .
            '<input id="amount_rows" type="text" name="wplp_amount_rows" value="' .
            esc_html(htmlspecialchars(isset($settings['amount_rows']) ? $settings['amount_rows'] : '')) .
            '" class="short-text" ' . esc_attr($classdisabled) . esc_attr($classdisabledsmooth) . '/></li>';

        /**
         * Pagination drop-down *
         */
        echo '<li class="field ' . esc_attr($classdisabled) . esc_attr($classdisabledsmooth) . '">
<label for="pagination" class="coltab">' . esc_html__('Pagination', 'wp-latest-posts') . '</label>' .
        ...
        foreach ($this->pagination_values as $value => $text) {
            echo '<option value="' . esc_html($value) . '" ' .
                (isset($pagination_selected[$value]) ? esc_html($pagination_selected[$value]) : '') . '>';
        ...
        echo '<li class="field"><label for="max_elts" class="coltab">' .
            esc_html__('Max number of elements', 'wp-latest-posts') . '</label>' .
            '<input id="max_elts" type="text" name="wplp_max_elts" value="' .
            esc_html(htmlspecialchars(isset($settings['max_elts']) ? $settings['max_elts'] : '')) .
            '" class="short-text" /></li>';
        /*
         * display total width
         */
        echo '<li class="field"><label for="total_width" class="coltab">' .
             esc_html__('Total width', 'wp-latest-posts') . '</label>' .
            '<input id="total_width" type="text" name="wplp_total_width" value="' .
            esc_html(htmlspecialchars(isset($settings['total_width']) ? (int)$settings['total_width'] : '')) .
            '" class="short-text" />';
...
*/
        ...
        foreach ($this->width_unit_values as $value => $text) {
            echo '<option value="' . (isset($value) ? esc_html($value) : '') . '" ' .
                (isset($units_selected[$value]) ? esc_html($units_selected[$value]) : '') . '>' .
        ...
        echo '<li class="field"><label for="off_set" class="coltab">' .
             esc_html__('Number of posts to skip:', 'wp-latest-posts') . '</label>' .
            '<input id="off_set" type="text" name="wplp_off_set" value="' .
            esc_html(htmlspecialchars(isset($settings['off_set']) ? (int)$settings['off_set'] : '')) . '" class="short-text" />';

        /**
 * Load posts in AJAX radio button set *
*/
        echo '<li class="field lazyloadConfig"><label class="coltab">' . esc_html__('Lazy load image', 'wp-latest-posts') .'</label>'.
            '<span class="radioset">' .
            '<input id="lazyload_default1" type="radio" name="wplp_layzyload_default" value="0" ' .
            (isset($layzyload_default_checked[0]) ? esc_html($layzyload_default_checked[0]) : '') . '/>' .
            '<label for="lazyload_default1">' . esc_html__('Off', 'wp-latest-posts') . '</label>' .
            '<input id="lazyload_default2" type="radio" name="wplp_layzyload_default" value="1" ' .
            (isset($layzyload_default_checked[1]) ? esc_html($layzyload_default_checked[1]) : '') . '/>' .
            '<label for="lazyload_default2">' . esc_html__('On', 'wp-latest-posts') . '</label>' .
            '</span>';
        echo '</li>';
        ...
        echo '<li class="field"><label class="coltab" style="vertical-align: top;">' .
             esc_html__('Button color', 'wp-latest-posts') . '</label>';
        echo '<div id="readmoreBgColor" style="display: inline-block">
            ...
            esc_html(htmlspecialchars(isset($settings['readmore_bg_color']) ? $settings['readmore_bg_color'] : 'transparent')) . '"/>
            </div>';
        echo '</li>';
        // button text color
        echo '<li class="field"><label class="coltab" style="vertical-align: top;">' .
             esc_html__('Button text color', 'wp-latest-posts') . '</label>';
        echo '<div id="readmoreTextColor" style="display: inline-block">
            ...
            esc_html(htmlspecialchars(isset($settings['readmore_text_color']) ? $settings['readmore_text_color'] : '#0c0c0c')) . '"/>
            </div>';
        echo '</li>';
        // Button size drop-down
        echo '<li class="field">
            <label for="readmore_size" class="coltab" style="vertical-align: middle;">' .
             esc_html__('Button size', 'wp-latest-posts') . '</label>' .
        ...
        */
        echo '<li class="field">
            <label for="readmore_border" class="coltab">' .
             esc_html__('Button border radius', 'wp-latest-posts') . '</label>' .
            '<input id="readmore_border" type="text" name="wplp_readmore_border" value="' .
            esc_html(htmlspecialchars(isset($settings['readmore_border']) ? $settings['readmore_border'] : '0')) .
            '" class="short-text" style="width: 10%"/>'.
            '<span class="readmore-border-param">'.esc_html__('px', 'wp-latest-posts') .'</span>'
            .'</li>';
        echo '</ul>';    //fields
        echo '</div>';    //wpcu-inner-admin-block
        // ---------------------------------------- //
        ...
        echo '<li class="field"><label class="coltab" style="vertical-align: middle;">' .
             esc_html__('Overlay color', 'wp-latest-posts') . '</label>';
        echo '<div id="overlayColor" style="display: inline-block">
            ...
            esc_html(htmlspecialchars(isset($settings['overlay_color']) ? $settings['overlay_color'] : 'transparent')) . '"/>
            </div>';
        echo '</li>';
        // overlay transparent
        echo '<li class="field"><label class="coltab" style="vertical-align: middle;">' .
             esc_html__('Overlay transparency', 'wp-latest-posts') . '</label>';
        echo '<span id="slider_overlay_transparent" class="overlay-slider"></span>' .
            ...
            esc_html(htmlspecialchars(isset($settings['overlay_transparent']) ? $settings['overlay_transparent'] : '0')) .
            '" class="short-text" />';
        echo '</li>';
        // overlay icon
        echo '<li class="field">
            <label for="overlayIcon" class="coltab" style="vertical-align: middle;">' .
             esc_html__('Overlay icon', 'wp-latest-posts') . '</label>' .
            '<input id="overlayIcon" class="btn" type="button" value="' .
             esc_html__('Choose an icon', 'wp-latest-posts') . '" />';
        echo '<input id="overlayIconSelected" name="wplp_overlay_icon_selected"
         type="text" readonly="true" value="'.
            esc_html(htmlspecialchars(isset($settings['overlay_icon_selected']) ? $settings['overlay_icon_selected'] : 'f109'))
            .'" class="short-text"/>';
        echo '</li>';
        // icon color
        echo '<li class="field"><label class="coltab" style="vertical-align: middle;">' .
             esc_html__('Icon color', 'wp-latest-posts') . '</label>';
        echo '<div id="overIconColor" style="display: inline-block">
            ...
            esc_html(htmlspecialchars(isset($settings['over_icon_color']) ? $settings['over_icon_color'] : '#ff0000')) . '"/>
            </div>';
        echo '</li>';
        // icon color
        echo '<li class="field"><label class="coltab" style="vertical-align: middle;">' .
             esc_html__('Icon background color', 'wp-latest-posts') . '</label>';
        echo '<div id="overBGIconColor" style="display: inline-block">
            ...
            esc_html(htmlspecialchars(isset($settings['over_bg_icon_color']) ? $settings['over_bg_icon_color'] : '#2C8FC7')) . '"/>
            </div>';
        echo '</li>';
        ?>
...
**/
        echo '<li class="field"><label class="coltab" style="vertical-align: top;">' .
             esc_html__('Arrow color', 'wp-latest-posts') . '</label>';
        echo '<div id="arrowColor" style="display: inline-block">
                  <input id="arrowColor" name="wplp_arrow_color" class="wplp_colorPicker" value="' .
            esc_html(htmlspecialchars(isset($settings['arrow_color']) ? $settings['arrow_color'] : 'rgb(51, 51, 51)')) . '"/>
            </div>';
        echo '</li>';
        echo '<li class="field"><label class="coltab" style="vertical-align: top;">' .
             esc_html__('Arrow mouse hover', 'wp-latest-posts') . '</label>';
        echo '<div id="arrowHoverColor" style="display: inline-block">
            ...
            esc_html(htmlspecialchars(isset($settings['arrow_hover_color']) ? $settings['arrow_hover_color'] : 'rgb(54, 54, 54)')) . '"/>
            </div>';
        echo '</li>';
        echo '</ul>';    //fields
        echo '</div>';    //wpcu-inner-admin-block
...
<label for="theme" class="coltab">' . esc_html__('Theme', 'wp-latest-posts') . '</label>' .
        ...
        if (!class_exists('WPLPAddonAdmin')) {
            echo '<div class="card wplp_notice light-blue notice notice-success is-dismissible below-h2">' .
                '<div class="card-content white-text">' .
                 esc_html__('Additional advanced customization features and various beautiful pre-configured templates and themes are available with the optional ', 'wp-latest-posts') .
                 '<a href="http://www.joomunited.com/wordpress-products/wp-latest-posts" 
                        target="_blank" >'.esc_html__('pro add-on', 'wp-latest-posts').'</a></div></div>';
        }
    }
    ...
    private function displayContentSourceTab()
    {
        ...
        <?php foreach ($tabs as $tabhref => $tab) : ?>
            <li><a href="#<?php echo esc_url($tabhref); ?>"
            id="<?php echo esc_html($tab['id']); ?>">
            <?php echo esc_html($tab['name']); ?></a></li>
        <?php endforeach; ?>
                        </ul>-->
        ...
        <!-- CHECK WPML or Polylang is INSTALLED AND ACTIVED -->
            ...
            <div class="content-source-language">
                <label for="content_language"
                       class="content-language-label"><?php esc_html_e('Content language', 'wp-latest-posts'); ?></label>
        ...
        <ul class="horizontal">
            <?php $idx = 0; ?>
            <?php foreach ($tabs as $tabhref => $tab) : ?>
                <li><input type="radio" name="wplp_source_type" id="sct<?php echo esc_html(++$idx); ?>"
                           value="<?php echo esc_html($tab['value']); ?>"
                           class="source_type_sel with-gap"
                        <?php echo(isset($source_type_checked[$tab['value']]) ?
                            esc_html($source_type_checked[$tab['value']]) : ''); ?> />
                    <label for="sct<?php echo esc_html($idx); ?>" class="post_radio"><?php echo esc_html($tab['name']); ?></label></li>
            <?php endforeach; ?>
        </ul>

        <?php foreach ($tabs as $tabhref => $tab) : ?>
        <div id="div-<?php echo esc_html($tab['value']); ?>" class="wplp_source_type_section">
    ...
    private function displayContentSourceCategoryListTab()
    {
        ...
        if (is_multisite()) {
            ...
            echo '<li class="field "> 
                ...
                esc_html__('Multisite selection', 'wp-latest-posts') . ' : </label>		
		                <select id="mutilsite_cat_list_select" class="mutilsite_select" name="wplp_mutilsite_cat_list">
				             <option value="all_blog">' . esc_html__('All blog', 'wp-latest-posts') . '</option>';
        ...
        echo '<input type="hidden" value="' . esc_html($mutilsite_selected_post) . '" id="selected_multisite_cat_list_post_type" />';
        ...
        echo '<li><input id="cat_list_all" type="checkbox" name="wplp_source_category_list[]" 
                value="_all" ' . (isset($source_cat_list_checked['_all']) ? esc_html($source_cat_list_checked['_all']) : '') . ' />' .
        ...
        if (is_multisite()) {
            ...
            foreach ($cats as $k => $cat) {
                echo '<li><input id="cl_' . esc_html($k) . '" type="checkbox" name="wplp_source_category_list[]" value="' .
                    esc_html($k . '_' . $cat->term_id . '_blog' . $cat->blog) .'" ' .
                    (isset($source_cat_list_checked[$k . '_' . $cat->term_id . '_blog' . $cat->blog ]) ?
                        esc_html($source_cat_list_checked[$k . '_' . $cat->term_id . '_blog' . $cat->blog ]) : '') .
                    ' class="cat_list_cb" />';
                echo '<label for="cl_' . esc_html($k) . '" class="cat_list_cb">' . esc_html($cat->name) . '</label></li>';
            }
        } else {
            $cats = get_categories(array('hide_empty' => false));
            ...
            foreach ($cats as $k => $cat) {
                echo '<li><input id="cl_' . esc_html($k) . '" type="checkbox" name="wplp_source_category_list[]" value="' .
                    ...
                    (isset($source_cat_list_checked[$cat->term_id]) ? esc_html($source_cat_list_checked[$cat->term_id]) : '') .
                    ' class="cat_list_cb" />';
                echo '<label for="cl_' . esc_html($k) . '" class="cat_list_cb">' . esc_html($cat->name) . '</label></li>';
            }
        }
        ...
        echo '<label for="cat_list_source_order" class="coltab">' . esc_html__('Order by', 'wp-latest-posts') . '</label>';
        echo '<select name="wplp_cat_list_source_order" id="cat_list_source_order" >';
        echo '<option value="id" ' .
            (isset($source_catlist_order_selected['id']) ? esc_html($source_catlist_order_selected['id']) : '') . '>' .
             esc_html__('By id', 'wp-latest-posts') . '</option>';
        echo '<option value="name" ' .
            (isset($source_catlist_order_selected['name']) ? esc_html($source_catlist_order_selected['name']) : '') . '>' .
             esc_html__('By name', 'wp-latest-posts') . '</option>';
        echo '<option value="description" ' .
            (isset($source_catlist_order_selected['description']) ? esc_html($source_catlist_order_selected['description']) : '') .
            '>' . esc_html__('By description', 'wp-latest-posts') . '</option>';
        ...
        echo '<label for="cat_list_source_asc" class="coltab">' .
             esc_html__('Posts sort order', 'wp-latest-posts') . '</label>';
        echo '<select name="wplp_cat_list_source_asc" id="cat_list_source_asc">';
        echo '<option value="asc" ' .
            (isset($source_catlist_asc_selected['asc']) ? esc_html($source_catlist_asc_selected['asc']) : '') . '>' .
             esc_html__('Ascending', 'wp-latest-posts') . '</option>';
        echo '<option value="desc" ' .
            (isset($source_catlist_asc_selected['desc']) ? esc_html($source_catlist_asc_selected['desc']) : '') . '>' .
             esc_html__('Descending', 'wp-latest-posts') . '</option>';
        echo '</select>';
        echo '</li>'; //field
        echo '</ul>'; //fields
        ...
        if (!class_exists('WPLPAddonAdmin')) {
            echo '<div class="card wplp_notice light-blue notice notice-success is-dismissible below-h2" >' .
                '<div class="card-content white-text">' .
                 esc_html__('Additional content source options are available with the optional ', 'wp-latest-posts').
                 '<a href="http://www.joomunited.com/wordpress-products/wp-latest-posts" target="_blank" >'.
                 esc_html__('pro addon', 'wp-latest-posts').
                 '</a></div></div>';
        }
    }
    ...
    private function displayContentSourceCategoryTab()
    {
        ...
        echo '<input type="hidden" value="' . esc_html($mutilsite_selected_post) . '" id="selected_multisite_post_type" />';
        echo '<ul class="fields">';

        echo '<li class="field postcat">';
        echo '<ul  class="post_field">';
        echo '<li><input id="cat_all" type="checkbox" name="wplp_source_category[]" 
value="_all" ' . (isset($source_cat_checked['_all']) ? esc_html($source_cat_checked['_all']) : '') . ' />' .
        ...
        if (is_multisite()) {
            ...
            foreach ($cats as $k => $cat) {
                echo '<li><input id="ccb_' . esc_html($k) . '" type="checkbox" name="wplp_source_category[]" value="' . esc_html($k . '_' .
                    $cat->term_id) . '" ' .
                    (isset($source_cat_checked[$k . '_' . $cat->term_id]) ?
                        esc_html($source_cat_checked[$k . '_' . $cat->term_id]) : '') .
                    ' class="post_cb" />';
                echo '<label for="ccb_' . esc_html($k) . '" class="post_cb">' . esc_html($cat->name) . '</label></li>';
            }
        } else {
            $cats = get_categories(array('hide_empty' => false));
            if (isset($settings['content_language'])) {
            ...
            foreach ($cats as $k => $cat) {
                echo '<li><input id="ccb_' . esc_html($k) . '" type="checkbox" name="wplp_source_category[]" value="' .
                    esc_html($cat->term_id) . '" ' .
                    (isset($source_cat_checked[$cat->term_id]) ? esc_html($source_cat_checked[$cat->term_id]) : '') .
                    ' class="post_cb" />';
                echo '<label for="ccb_' . esc_html($k) . '" class="post_cb">' . esc_html($cat->name) . '</label></li>';
            }
        }
        ...
        echo '<li class="order_field field input-field input-select">';
        echo '<label for="cat_post_source_order" class="coltab">' . esc_html__('Order by', 'wp-latest-posts') . '</label>';
        ...
        if (!class_exists('WPLPAddonAdmin')) {
            echo '<div class="card wplp_notice light-blue notice notice-success is-dismissible below-h2" >' .
                '<div class="card-content white-text">' .
                 esc_html__('Additional content source options are available with the optional ', 'wp-latest-posts').
                    '<a href="http://www.joomunited.com/wordpress-products/wp-latest-posts" target="_blank" >'.
                 esc_html__('pro addon', 'wp-latest-posts').'</a></div></div>';
        }
    }
    ...
    private function displayContentSourcePageTab()
    {
        ...
        echo '<input type="hidden" value="' . esc_html($mutilsite_selected_page) . '" id="selected_multisite_page_type" />';
        echo '<ul class="fields">';
        ...
        echo '<label for="pg_source_order" class="coltab">' . esc_html__('Order by', 'wp-latest-posts') . '</label>';
        echo '<select name="wplp_pg_source_order" id="pg_source_order" >';
        echo '<option value="order" ' .
            (isset($source_order_selected['order']) ? esc_html($source_order_selected['order']) : '') . '>' .
             esc_html__('By order', 'wp-latest-posts') . '</option>';
        echo '<option value="title" ' .
            (isset($source_order_selected['title']) ? esc_html($source_order_selected['title']) : '') . '>' .
             esc_html__('By title', 'wp-latest-posts') . '</option>';
        echo '<option value="date" ' .
        ...
        echo '<li class="order_dir field input-field input-select">';
        echo '<label for="pg_source_asc" class="coltab">' . esc_html__('Pages sort order', 'wp-latest-posts') . '</label>';
